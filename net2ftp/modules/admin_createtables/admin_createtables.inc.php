...
// ------------------------------------
		if ($mydb != false) {
			$mysqli_select_db_result = mysqli_select_db($mydb, $dbname2);
			if ($mysqli_select_db_result == false) { $net2ftp_output["admin_createtables"][] = __("Unable to select the database <b>%1\$s</b>.", $dbserver2_html) . "\n"; }
		}

// ------------------------------------
// Query
// ------------------------------------
		if ($mydb != false && $mysqli_select_db_result != false) {
			for ($i=0; $i<sizeof($sqlquerypieces); $i++) {
				$mysqli_query_results[$i] = mysqli_query($mydb, $sqlquerypieces[$i]);
